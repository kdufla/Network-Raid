...
void send_info_path_multiple(int *info, const char *path)
{
	...
	{
		send(sfds[i], info, sizeof(int) * INFO_SIZE, MSG_NOSIGNAL);
		send(sfds[i], path, info[1], MSG_NOSIGNAL);
	}
}
...
static int do_read4(const char *path, char *buf, size_t size, off_t offset,
...
void *timer_function5(void *x_void_ptr)
{
	int info[INFO_SIZE] = {check_num, 0, 0, 0, 0, 0};
	while (true)
	{
		pthread_mutex_lock(&mutex5);
		int i;
		for (i = 0; i < num_servers; i++)
		{
			send(sfds[i], info, sizeof(info), MSG_NOSIGNAL);
		}
		pthread_mutex_unlock(&mutex5);
...
int main_raid5(int argc, char *argv[], storage *stor)
{
	...
	int rv = fuse_main(argc, argv, &do_oper5, NULL);
	return rv;
}
